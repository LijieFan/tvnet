...
class Tvl1Flow(object):
    ...
    def warp_image(self, x, u, v):
        ...
        # assert u.shape[0].value == tf.shape(x)[0]
        # assert v.shape[0].value == tf.shape(x)[0]
    ...
    def forward_gradient(self, x, name):
	    ...
	    diff_y = tf.concat(axis=1, values=[diff_y_valid, last_row])
            ...
            '''mask_np = np.zeros([1, x.shape[1].value, x.shape[2].value, x.shape[3].value],
                               dtype=np.float32)

            mask_np[:, :, -1, :] = 1
            mask = tf.constant(mask_np)
            mask = tf.tile(mask, [tf.shape(x)[0],1,1,1])
            diff_x = (1 - mask) * diff_x
            ...
            with tf.variable_scope('warping%d' % (warpings,)):
                ...
                for ii in xrange(max_iterations):
                    with tf.variable_scope('iter%d' % (ii,)):
                        ...
                        '''g1 = tf.sqrt(tf.square(u1x) + tf.square(u1y) + self.GRAD_IS_ZERO)
                        g2 = tf.sqrt(tf.square(u2x) + tf.square(u2y) + self.GRAD_IS_ZERO)
                                              
			ng1 = 1.0 + taut * g1
                        ng2 = 1.0 + taut * g2
			
			p11 = (p11 + taut * u1x) / (1.0 + taut * g1)
			p12 = (p12 + taut * u1y) / (1.0 + taut * g1)
			p21 = (p21 + taut * u2x) / (1.0 + taut * g2)
			p21 = (p22 + taut * u2y) / (1.0 + taut * g2)'''
